...
load_icode(int fd, int argc, char **kargv) {
    ...
    uint32_t total_strlen = 0;
    int i;
    for (i = 0; i < argc; ++i) {
      total_strlen += strnlen(kargv[i], EXEC_MAX_ARG_LEN) + 1;
    }

    uintptr_t strtop = USTACKTOP - (total_strlen / 4 + 1) * 4;
    uintptr_t argtop = strtop - argc * sizeof(char *);
    for (i = 0; i < argc; ++i) {
        strcpy((char *)strtop, kargv[i]);
        ((uint32_t *)argtop)[i] = strtop;
        strtop += strnlen(kargv[i], EXEC_MAX_ARG_LEN) + 1;
    }
